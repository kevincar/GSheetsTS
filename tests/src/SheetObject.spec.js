function sheetObjectTap(tap) {
    ...
    tap.test("getData should work", function (t) {
        var destructed = mouse.getData();
        t.deepEqual(destructed, data, "getData mouse data for write");
    });
    tap.test("convertFromGDate", function (t) {
        var dateTest = function (dateString, dateNumber) {
            var dateExpected = new Date(dateString);
            var dateObserved = SheetObject.convertFromGDate(dateNumber);
    ...
    tap.test("convertToGDate", function (t) {
